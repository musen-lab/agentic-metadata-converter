...
```json
{
  "analysis": {
    ...
    "legacy_value": "field_value",
    "mapping_results": [
      {
        "target_field": "target_field_name",
        "target_value": "transformed_value_or_original",
        "transformation_required": true/false,
        "transformation_description": "description of transformation",
        "field_similarity_score": 0.0-1.0,
        "value_compatibility_score": 0.0-1.0,
        "confidence_score": 0.0-1.0,
        "warnings": ["list of warnings if any"]
      }
      // Only include mappings with confidence_score >= 0.6, ordered highest to lowest
    ],
    "recommended_mappings": [
      {
        "target_field": "best_target_field_name",
    ...
    "mapping_strategy": "one-to-one/one-to-many",
    "reasoning": "explanation of why this is the best mapping approach",
    "alternative_mappings": [
      // All mappings from mapping_results that are NOT in recommended_mappings
      // These represent other viable options with confidence_score >= 0.6
    ],
    "no_mapping_reason": "explanation if no viable mapping found"
  }
}
```
